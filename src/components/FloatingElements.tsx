...
export default function FloatingElements() {
  const { currentSection, totalSections } = useScrollSection();
  const pathname = usePathname();

  // Check if we're on the thank-you page
  const isThankYouPage = pathname === "/thank-you";

  // For thank-you page, show static 08/08
  const displayTotal = isThankYouPage
    ? "08"
    : totalSections.toString().padStart(2, "0");
  const displayCurrent = isThankYouPage
    ? "08"
      ...
      <div className="mx-auto px-4 lg:px-12 h-full relative w-full max-w-full">
        ...
        <div className="absolute left-4 lg:left-8 bottom-8 flex flex-col space-y-6 pointer-events-auto">
          <a
            ...
            aria-label="Send email to Mariel Genodiala"
          >
            <svg
              className="w-8 h-8"
              fill="none"
          ...
          <a
            href="https://www.linkedin.com/in/mariel-genodiala-059237231/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-white hover:text-dim-gray transition-colors"
            aria-label="Visit Mariel Genodiala's LinkedIn profile (opens in new tab)"
          >
          ...
          <a
            href="https://github.com/marielgenodiala"
            target="_blank"
            rel="noopener noreferrer"
            className="text-white hover:text-dim-gray transition-colors"
            aria-label="Visit Mariel Genodiala's GitHub profile (opens in new tab)"
          >
            <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="absolute right-4 lg:right-8 bottom-8 flex flex-col items-center pointer-events-auto ">
          ...
          <div className="w-[3px] h-28 lg:h-44 bg-white/20 my-3"></div>
          <div className="text-white/20 text-lg md:text-xl font-medium transform -rotate-90">
            {displayCurrent}
          </div>
        </div>
